...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    #[ORM\OneToMany(mappedBy: 'createdBy', targetEntity: Projet::class, orphanRemoval: true)]
    private Collection $projets;

    #[ORM\OneToMany(mappedBy: 'fundedBy', targetEntity: Financer::class)]
    private Collection $financers;

    #[ORM\Column(length: 255)]
    private ?string $Monnaie = "500";
    ...
    public function removeFinancer(Financer $financer): self
    {
        if ($this->financers->removeElement($financer)) {
            // set the owning side to null (unless already changed)
            if ($financer->getFundedBy() === $this) {
                $financer->setFundedBy(null);
            }
        }
